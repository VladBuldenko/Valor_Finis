...
from tests.helpers import auth_headers, create_account, create_income


# Tests that POST /income with account_id creates a linked Income and the
# response reports the linkage.
# Parameters:
...
#   Account's balance (via GET /accounts) reflects the credit.
def test_create_income_endpoint_linked_happy_path(
    ...
    response = client.post(
        ...
        json={
            "amount": "2500.00", "currency": "EUR", "received_at": "2026-09-30",
            "source": "salary", "description": None, "account_id": account["id"],
        },
    )
    ...
    assert accounts_response.json()[0]["current_balance"] == "2500.00"


# Tests that POST /income with an archived Account's id is rejected with
# 409 and the exact existing Account error message.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
# - None. The test passes if the response is 409 with that message.
def test_create_income_endpoint_archived_account_rejected(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")
    ...
    response = client.post(
        ...
        headers=auth_headers(user_id),
        json={
            "amount": "100.00", "currency": "EUR", "received_at": "2026-09-30",
            "source": "salary", "description": None, "account_id": account["id"],
        },
    )
...
# Tests that POST /income with a currency-mismatched Account is rejected
# with 422 and the exact documented error message.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
# - None. The test passes if the response is 422 with that message.
def test_create_income_endpoint_currency_mismatch_rejected(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
...
# Tests attach/detach/move via PATCH /income/{id}.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
...
def test_update_income_endpoint_attach_detach_move(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    ...
    income = create_income(client=client, user_id=user_id, amount=300, currency="EUR")
    ...
    assert accounts[account_a["id"]]["current_balance"] == "300.00"
    ...
    assert accounts[account_b["id"]]["current_balance"] == "300.00"
...
# Tests that GET /income exposes account_id correctly for a mix of
...
def test_get_income_endpoint_exposes_account_id(client: TestClient, clean_database: None) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")

    client.post(
        "/api/v1/income", headers=auth_headers(user_id),
        json={
            "amount": "100.00", "currency": "EUR", "received_at": "2026-09-01",
            "source": "salary", "description": None, "account_id": account["id"],
        },
    )
    create_income(client=client, user_id=user_id, amount=50, currency="EUR", source="gift")
...
# Tests that deleting a linked Income cascades its projection and the
...
def test_delete_income_endpoint_linked_cascades_projection(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")
    ...
    create_response = client.post(
        "/api/v1/income", headers=auth_headers(user_id),
        json={
            "amount": "400.00", "currency": "EUR", "received_at": "2026-09-01",
            "source": "salary", "description": None, "account_id": account["id"],
        },
    )
    ...
    delete_response = client.delete(
        f"/api/v1/income/{income_id}", headers=auth_headers(user_id),
    )
...
# Tests that another user cannot link their Income to someone else's
# Account.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
...
def test_create_income_endpoint_cross_user_account_not_found(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    ...
    response = client.post(
        ...
        json={
            "amount": "100.00", "currency": "EUR", "received_at": "2026-09-01",
            "source": "salary", "description": None, "account_id": other_account["id"],
        },
    )
    assert response.status_code == 404


# Tests that an Account's currency becomes immutable once a linked Income
# has established transaction history - reusing the existing, unmodified
# has_transactions_for_account check, now automatically covering
# Income-backed rows too.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
# - None. The test passes if the currency change is rejected with 409.
def test_account_currency_immutable_after_linked_income(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")
...
# Tests that an Account with linked-Income history cannot be hard-deleted
# - reusing the existing, unmodified has_transactions_for_account check.
# Parameters:
# - client: FastAPI test client.
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
# - None. The test passes if the delete is rejected with 409.
def test_account_hard_delete_blocked_by_linked_income_history(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")
...
# Tests that GET /accounts/{id}/transactions exposes kind="income" and
# income_id for a linked-Income-backed row, and null income_id for a
...
# - clean_database: fixture that clears database tables before and after the test.
# Returns:
# - None. The test passes if both rows have the expected kind/income_id
#   shape.
def test_account_history_endpoint_exposes_income_kind_and_income_id(
    client: TestClient, clean_database: None,
) -> None:
    user_id = str(uuid4())
    account = create_account(client=client, user_id=user_id, currency="EUR")
    ...
    client.post(
        ...
        headers=auth_headers(user_id),
        json={"direction": "credit", "amount": "10.00", "transaction_date": "2026-09-02"},
    )
    ...
    assert by_kind["income"]["income_id"] == income_id
    assert by_kind["adjustment"]["income_id"] is None
