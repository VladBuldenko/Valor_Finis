...
# Tests that the repository raises an error for duplicate budgets.
# This test exists to verify that duplicate budget conflicts are handled at the repository layer.
# Parameters:
# - clean_database: Fixture that cleans database tables before and after the test.
# Returns:
# - None. The test passes if the repository raises BudgetAlreadyExistsError.
def test_create_budget_raises_error_for_duplicate_budget(
    clean_database: None,
) -> None:
    # Arrange
    db_session = SessionLocal()
